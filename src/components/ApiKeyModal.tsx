...
export const ApiKeyModal: React.FC<ApiKeyModalProps> = ({
  ...
  const handleTestAndSave = async () => {
    ...
    // Reject obviously fake / incomplete keys before hitting the network
    // Real Google AI Studio keys start with "AIza" and are ~39 characters
    if (trimmed.length < 20 || !/^AIza[0-9A-Za-z_\-]{20,}$/.test(trimmed)) {
      setValidationStatus('invalid');
      setValidationError(
        'That does not look like a valid Gemini API key. Keys from Google AI Studio start with "AIza…".'
      );
      return;
    }
    ...
    try {
      ...
      // ONLY unlock when the server explicitly confirms a live Gemini call succeeded
      if (res.ok && data.valid === true) {
        ...
        setValidationError(
          data.error ||
            'Invalid API key. Get a key from Google AI Studio (aistudio.google.com/apikey) and try again.'
        );
      }
    } catch (err: any) {
      // Network / proxy failure — do NOT unlock with an unverified key
      setValidationStatus('invalid');
      setValidationError(
        err?.message
          ? `Could not reach validation server: ${err.message}`
          : 'Could not verify the key (network error). Check that the backend is running and try again.'
      );
    } finally {
      setValidating(false);
    }
